                ...
                @foreach($sidebar as $m)
                    ...
                    @foreach($m['sub-link'] as $link)
                        <div aria-label="Dashboards" class="mt-3 space-y-4 px-2" role="menu" x-show="open">
                            <a class="dark:shadow-soft-light-2xl dark:hover:shadow-soft-light-sm
                                shadow-soft-3xl hover:shadow-soft-sm
                                block flex p-2 text-sm text-gray-400 transition-all text-primary-light duration-200 rounded-md
                                dark:hover:text-light hover:text-gray-700 bg-white rounded-md  transition-all transition ease-in-out
                                hover:bg-primary-100 dark:bg-darker dark:hover:bg-primary hover:-translate-y-1"
                               href="{{ $link['link'] }}" role="menuitem">
                                <span
                                ...
                                <span class="flex ml-1 align-middle inline-block p-1 font-bold ">{{ $link['title'] }}</span>
                            </a>
                        </div>
                    @endforeach
                @endforeach
